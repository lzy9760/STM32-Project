...
void GM6020_CAN_Init(void)
{
    ...
    //使能时钟（AFIO必须开启，否则GPIO复用失效）
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_AFIO, ENABLE);
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_CAN1, ENABLE);

    //配置GPIO（PA11=CAN_RX，PA12=CAN_TX）
...
void GM6020_Send_Voltage(int32_t voltage)
{
    ...
    // 计算对应ID的电压数据偏移（ID1→0/1字节，ID2→2/3字节，ID3→4/5字节）
...
void GM6020_Parse_Feedback(CanRxMsg* rx_msg)
{
    ...
    gm6020_total_angle = (gm6020_round_cnt * 8192 + mechanical_angle) * 360.0f / 8192.0f;  // 总角度（度）

    // 更新状态变量
    gm6020_status.mechanical_angle = mechanical_angle;
    gm6020_status.speed_rpm = speed_rpm;
    gm6020_status.actual_current = (float)current_raw / 16384.0f * 3.0f;  // 电流换算（-3A~3A）
