...
录。为了便于理解，可将其划分为五层。
层级 主要文件 职责 输入/输出关系
系统初始化、任务创建、自动 调用各模块 Init/Task，决定导
应用入口层 app.c, app_freertos.c
导航入口、手动接管仲裁 航或遥控哪个控制底盘
app_nrf24l01.c,
NRF 遥控接收、摇杆解析、失 遥控包->运动命令；位姿/NRF
通信交互层 app_nrf_remote.c,
联保护、OLED 状态显示 状态->OLED
app_remote.c, app_oled.c
app_encoder.c,
app_mpu6050.c,
编码器读取、IMU 滤波与姿态
感知定位层 app_imu_filter.c, 轮增量/yaw->X/Y/yaw
估计、轮惯融合里程计
app_imu_ahrs.c,
app_odometry.c
app_path_planner.c, 生成路径、采样目标、估计进 目标点+当前位姿->车体速度
规划跟踪层
app_navigation.c 度、路径跟踪、终点控制 命令
app_chassis.c, app_motor.c, 麦轮逆运动学、PWM 方向控 vx/vy/w->四轮目标->PWM 输
执行控制层
pid.c, pid.h 制、四轮 PID 速度闭环 出
...
Remote_ControlTask,
底盘；自动导航运行时
Remote_Task 5 ms 4 Remote_HasManualC
持续监测是否有人为输
ommand
入
自动导航闭环，读取位
Navigation_TaskEntry 5 ms 4 Navigation_Task
姿并输出底盘运动命令
独立维持四轮速度闭
Pid_Task 10 ms 3 Pid_UpdateAll
环，减小电机差异影响
显示状态和坐标，优先
Oled_Task 50 ms 1 AppOled_Task 级最低，避免阻塞控制
链路
...
IMU_AHRS_INTEGRAL_MAX/
±0.05 积分限幅 防止异常状态下过修正
MIN
...
状态字段 含义 运行中的作用
为 0 时 Navigation_Task 直接返回，为 1
active 导航是否正在运行
时进入闭环控制
目标方向与当前 yaw 差大于阈值时使用曲
follow_path_yaw 是否按曲线路径方向跟随
线跟踪
...
CHASSIS_*_GAIN,
轮径/摩擦/电机差异或航向偏
直线总偏一侧 ODOM_FORWARD_SCALE, 先测纯前进，再逐轮微调增益
差
yaw 零偏
侧移距离不准 ODOM_STRAFE_SCALE 麦轮侧向滑移比前进更明显 左右侧移分别测多次取平均
gyro bias、
陀螺仪零偏未完全收敛或静止 上电保持静止；调小静止误判
静止 yaw 缓慢漂移 yaw_bias_rad_s、静止检测
判断过松 概率；检查机械振动
阈值
AKF 测量方差 R、创新滑窗、 原始陀螺仪噪声或线缆/供电干 先观察原始角速度，再确认滤
IMU 输出抖动
MPU6050 DLPF 扰 波后曲线是否平滑
NAV_LOOKAHEAD_MM,
曲线跟踪抖动 前瞻太短或横向反馈太强 适当增大前瞻，减小横向 Kp
NAV_CROSS_TRACK_KP
NAV_LOOKAHEAD_MM, 前瞻过长，车辆追远点而抄近
曲线切弯明显 减小前瞻或提高横向纠偏
NAV_CROSS_TRACK_KP 路
PATH_ACCEL_RATIO,
降低速度、增大减速距离、减
终点过冲 NAV_FINAL_SPEED_LIMIT, 末端速度过大或地面打滑
小终点限幅
最大速度

NAV_YAW_DEADBAND_DEG,
终点附近转圈 NAV_FINAL_YAW_DISABLE_ 角度控制与位置控制耦合 增大死区或提前关闭 yaw 修正
MM
加入 UWB/视觉/AprilTag 并使
远距离误差增大 定位传感器方案 相对里程计误差累计
用 EKF 融合
