...
<h1><b>STM32 轮惯融合全向移动机器人<br/>高精度相对定位与自动导航系统</b></h1>
...
这是一个基于 STM32 的四轮麦克纳姆轮移动机器人项目。更准确地说，它不是严格意义上的纯惯性导航，而是“轮式里程计 + IMU”的轮惯融合相对定位与航位推算系统：四路编码器负责估计平面位移，MPU6050 负责提供姿态角和航向辅助，最终在车端实时解算 X/Y/yaw 并用于自动导航闭环控制。

当前主分支只保留这个最完整、最适合展示的项目；之前仓库中原有的 STM32 学习项目已经保留到次级分支：

```text
legacy-stm32-learning-projects
```

当前项目已经完成 NRF24L01 遥控、四路编码器速度闭环、MPU6050 姿态估计、轮惯融合相对里程计、车端 OLED 实时坐标显示、自动定位导航和手动遥控接管。实测不打滑情况下 3 米级自动导航平均误差小于 3 cm。
...
| 定位 | 轮惯融合航位推算，四轮编码器里程计 + IMU yaw，输出相对 X/Y/yaw |
